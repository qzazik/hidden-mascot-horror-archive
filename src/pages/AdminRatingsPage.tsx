...
import type { ArchiveStatus, EditorialRatingData, Game, RatingConfidence, ReviewProgress, VerificationStatus } from '../types';
import { games } from '../data/library';
import { useLocalStorageState } from '../hooks/useLocalStorage';
import { allRatingCriteria, archiveStatusText, calculateOverallRating, defaultReviewProgress, deriveArchiveStatus, getRatingRank, normalizeRatingSet, ratingGroups } from '../utils/ratings';
...
const progressFields: Array<{ key: keyof ReviewProgress; label: string }> = [
  { key: 'storePage', label: 'Страница игры' }, { key: 'screenshots', label: 'Скриншоты' }, { key: 'trailer', label: 'Трейлер' },
  { key: 'partialGameplay', label: 'Часть прохождения' }, { key: 'fullGameplay', label: 'Полное прохождение' },
  { key: 'technicalState', label: 'Техническое состояние' }, { key: 'streamSuitability', label: 'Потенциал для стрима' }
];
const makeDraft = (game: Game, override?: EditorialRatingData): EditorialRatingData => ({
  ratings: normalizeRatingSet(override?.ratings ?? game.ratings), ratingSummary: override?.ratingSummary ?? game.ratingSummary ?? null,
  strengths: override?.strengths ?? game.strengths ?? [], weaknesses: override?.weaknesses ?? game.weaknesses ?? [], recommendedFor: override?.recommendedFor ?? game.recommendedFor ?? [], streamVerdict: override?.streamVerdict ?? game.streamVerdict ?? null,
  verificationStatus: override?.verificationStatus ?? game.verificationStatus, ratingConfidence: override?.ratingConfidence ?? game.ratingConfidence, reviewedAt: override?.reviewedAt ?? game.lastChecked,
  archiveStatus: override?.archiveStatus ?? deriveArchiveStatus(game), reviewProgress: override?.reviewProgress ?? game.reviewProgress ?? defaultReviewProgress(game)
});
...
export function AdminRatingsPage() {
  ...
  const [archiveFilter, setArchiveFilter] = useState<'all' | ArchiveStatus>('all');
  ...
  const filteredGames = useMemo(() => games.filter((game) => {
    ...
    if (archiveFilter !== 'all' && current.archiveStatus !== archiveFilter) return false;
    return true;
  }), [archiveFilter, filter, query, records]);
  ...
  return <div className="container ratings-admin">
    ...
    <div className="ratings-admin__layout">
      <aside className="ratings-admin__games panel"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск игры" /><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="all">Все игры</option><option value="unrated">Без оценки</option><option value="medium">Частично проверено</option><option value="high">Проверено</option></select><select value={archiveFilter} onChange={(event) => setArchiveFilter(event.target.value as typeof archiveFilter)}><option value="all">Любой архивный статус</option>{Object.entries(archiveStatusText).map(([key, value]) => <option value={key} key={key}>{value.label}</option>)}</select><div className="ratings-admin__game-list">{filteredGames.map((game) => { const value = records[game.slug] ?? makeDraft(game); const rating = calculateOverallRating(value.ratings); return <button type="button" className={game.slug === selectedGame.slug ? 'is-active' : ''} onClick={() => setSelectedSlug(game.slug)} key={game.slug}><span><strong>{game.title}</strong><small>{rating === null ? 'Без оценки' : `${rating.toFixed(1)} · ${getRatingRank(rating)}`}</small></span><span className="badge">{archiveStatusText[value.archiveStatus].label}</span></button>; })}</div></aside>
